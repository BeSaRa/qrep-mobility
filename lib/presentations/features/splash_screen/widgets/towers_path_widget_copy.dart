...
class RPSCustomPainter extends CustomPainter {
  List<Path> allPaths = List.generate(365, (index) => Path());
  List<Color> pathColors = List.generate(365, (index) => Colors.white);

// Step 2: Wrap in a model if needed

// Step 3: Initialize them for control

  @override
  void paint(Canvas canvas, Size size) {
    List<AnimatedPath> animatedPaths = List.generate(
      allPaths.length,
      (index) => AnimatedPath(
        path: allPaths[index],
        color: ColorManager.primary, // Default color, change dynamically
        id: index,
      ),
    );
    for (int i = 0; i < allPaths.length; i++) {
      final paint = Paint()
        ...
        ..color = pathColors[i].withOpacity(1.0); // dynamic color

      canvas.drawPath(allPaths[i], paint);
    }
